...
def admin_list(request):
    ...
    if not request.user.is_superuser:
        return JsonResponse({"error": "Bạn không có quyền để xem!"}, status=403)

    # Lấy danh sách các tài khoản admin
    users = User.objects.filter(is_superuser=True)  # Lọc ra các tài khoản admin

    # Tạo danh sách dữ liệu JSON từ các đối tượng user
    users_data = list(users.values('id', 'username', 'email', 'full_name'))
